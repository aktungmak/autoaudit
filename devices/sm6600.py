...
description = "SM6600 modulator"

lg = logging.getLogger('autoaudit')

class Device(object):
    def __init__(self, ipa):
        lg.debug('creating new sm6600 at %s', ipa)
        self.info = {
            'productname'  : None,
            'unitname'     : None,
            'ipaddress'    : ipa,
        ...
        # xpaths are stored as (xpath, attrib) due to etree not having .//@value syntax
        self.xpaths = {
            'unitname'     : ('.//Str[@N="Name"]', 'V'),
            'sntpserver'   : ('.//Table[@N="SNTP Server Table"]/R', 'F1'),
            'swversion'    : ('.//Str[@N="Product Version"]', 'V')
        }
    ...
    def populate(self):
        ...
        for k, v in self.info.iteritems():
            if k in self.oids:
                ...
                temp = self.configp.find(self.xpaths[k][0])
                if temp is not None:
                    self.info[k] = temp.attrib[self.xpaths[k][1]]
            else:
    ...
    def _getOptionCards(self):
        ...
        for snum in snums:
            ...
            self.optioncards["%02d - %s" % (tc.get('slotnum', '???'), tc.get('type', '???'))] = tc
