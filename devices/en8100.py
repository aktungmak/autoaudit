...
description = "VPC and AVP encoder"

lg = logging.getLogger('autoaudit')

class Device(object):
    def __init__(self, ipa):
        lg.debug('creating new en8100 at %s', ipa)
        self.info = {
            'productname'  : 'ViPENC',
            'unitname'     : None,
            'ipaddress'    : ipa,
        ...
        self.xpaths = {
            ...
            'servicename'  : './/outputList//serviceName'
        }
    ...
    def populate(self):
        ...
        for k, v in self.info.iteritems():
            if k in self.oids:
                ...
                rl = self.configp.findall(self.xpaths[k])
                if rl != []:
                    self.info[k] = u", ".join( map( lambda x: x.attrib.get('value', '-'), rl))
            else:
...
if __name__ == '__main__':
    # d = Device('192.168.32.167')
    # wb3
    d = Device('10.1.23.61')
    # d = Device('192.168.103.9')
    print d.dumpToJson()
